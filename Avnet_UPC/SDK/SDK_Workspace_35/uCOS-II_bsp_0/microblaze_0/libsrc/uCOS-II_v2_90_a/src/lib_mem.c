...
*
*                                   0,                          Default    alignment to single-
*                                                                   octet word boundary.
*                                   Non-zero,                   Otherwise, alignment to requested 
*                                                                   octet word boundary.
*
                      ...
                      LIB_ERR     *perr)
{
    ...
    align_actual = (align < 1u) ? 1u                                /* If dflt, align to single-octet word boundary; .. */
                                : align;                            /* .. else  align to req'd  octet word boundary.    */
    ...
    size_req  = Mem_PoolSegCalcTotSize((void     *)pmem_addr,
                                       (CPU_SIZE_T)1u,              /* Calc alloc for single mem blk from heap.         */
                                       (CPU_SIZE_T)size,
                                       (CPU_SIZE_T)align_actual);
    ...
    pmem_blk = Mem_PoolSegAlloc(pmem_pool_heap, size, align_actual);
                      ...
                      LIB_ERR     *perr)
{
    ...
    blk_align_actual = (blk_align < 1u) ? 1u                        /* If dflt, align to single-octet word boundary; .. */
                                        : blk_align;                /* .. else  align to req'd  octet word boundary.    */
...
/*$PAGE*/
    ...
    if (pmem_base_addr == (void *)0) {                              /* If no base addr, cfg mem pool from heap.         */
        ...
        pmem_base_addr_end   = (CPU_INT08U *)pmem_base_addr + mem_size - 1u;
        ...
        while (pmem_pool_next != (MEM_POOL *)0) {                   /* Srch tbl for mem seg with same base addr/size.   */
            ...
            if ((pmem_base_addr == pmem_pool_next->SegAddr) &&
                ...
                pmem_seg_addr_end   = (CPU_INT08U *)pmem_pool_next->SegAddr + pmem_pool_next->SegSizeTot - 1u;
...
/*$PAGE*/
    ...
    for (i = 0u; i < blk_nbr; i++) {                                /* Alloc mem blks from blk seg ptr.                 */
        pmem_blk = (void *)Mem_PoolSegAlloc(pmem_pool_blk, blk_size, blk_align_actual);
...
/*$PAGE*/
    ...
    pmem_pool->PoolAddrEnd   = (void       *)(pmem_addr_pool + size_tot_pool - 1u);
    pmem_pool->PoolPtrs      = (void      **) ppool_ptr;
    pmem_pool->PoolSize      = (CPU_SIZE_T  ) size_tot_pool;
    pmem_pool->BlkAlign      = (CPU_SIZE_T  ) blk_align_actual;
...
#else
   (void)&size;                                                     /* Prevent 'variable unused' compiler warning.      */
#endif

    if (pmem_pool->BlkIx < 1u) {
        CPU_CRITICAL_EXIT();
                                            ...
                                            CPU_SIZE_T   blk_align)
{
    ...
    size_tot = mem_align_offset + ((blk_size + blk_align_offset) * (blk_nbr - 1u)) + blk_size;
