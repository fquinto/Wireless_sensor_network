...
void transmitDataString(char addr[2], signed char rssi, char msg[MESSAGE_LENGTH], int uart )
{
  ...
  if(uart)
  {
    char output_verbose[] = {"INode:XX,Temp:-XX.XC,Battery:X.XV,Strength:XXX%\r\n"};
	
	/* Node */
	output_verbose[6] = addr[0];
    output_verbose[7] = addr[1];
    
    /* Temperature */
    output_verbose[14] = temp_string[0]; /* sign */
    output_verbose[15] = temp_string[1];
    output_verbose[16] = temp_string[2];
    output_verbose[17] = temp_string[3];
	...
	serial_log( output_verbose );
  }
  else
  {
    char output_short[] = {"Gxx,-XX.X,VV,RSI"};
	
	/* Node */
    output_short[1] = addr[0];
    output_short[2] = addr[1];
    
    /* Temperature */
    output_short[4] = temp_string[0]; /* sign */
    output_short[5] = temp_string[1];
    output_short[6] = temp_string[2];
    output_short[7] = temp_string[3];
    output_short[8] = temp_string[4];
	
	/* Batery voltage */
    output_short[10] = '0'+(msg[2]/10)%10;
    output_short[11] = '0'+(msg[2]%10);
	
	/* Strength signal: RSSI */
    output_short[13] = rssiString[2];
    output_short[14] = rssiString[1];
    output_short[15] = rssiString[0];
	
	toRadio( output_short );
  }
}
